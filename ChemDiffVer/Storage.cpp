...
void Storage::print_VTK_File(void) {
	...
	if ((SYSTEM)) {
		...
		std::string Number;
		//std::string initial = "Animation_realistic/YB_chemdiff_uPow8_ins0d8_LSim_fixBeta_ssNoUpdate_";
		std::string initial = "Animation_realistic/Test_on_new_cluster_";
		// std::string initial = "Animation_realistic/YB_chemdiffMech_uPow5To10In4e5StepsExp_LSim_fixBeta_ssNoUpdate_";
		// std::string initial = "Animation_realistic/YB_chemdiff_mechanical_verification_";
		std::ofstream ofs;
...
void Storage::storeVariables(void) {
	...
	if (SYSTEM) {
		...
		//std::string initial = "Variables_realistic/YB_chemdiff_uPow8_ins0d8_LSim_fixBeta_ssNoUpdate_";//yeastbudding_septin40_test_6particle_1pullonly_";
		std::string initial = "Variables_realistic/Test_on_new_cluster_";
		// std::string initial = "Variables_realistic/YB_chemdiffMech_uPow5To10In4e5StepsExp_LSim_fixBeta_ssNoUpdate_";
		// std::string initial = "Variables_realistic/YB_chemdiff_mechanical_verification_";
		std::ofstream ofs;
		if (digits == 1 || digits == 0) {
			Number = "0000" + std::to_string(iteration2);
		}
		else if (digits == 2) {
			Number = "000" + std::to_string(iteration2);
		}
		else if (digits == 3) {
			Number = "00" + std::to_string(iteration2);
		}
		else if (digits == 4) {
			Number = "0" + std::to_string(iteration2);
		}
		...
		// }
		double x,y,z,chem,b,c;
		for (int i = 0; i < SYSTEM->coordInfoVecs.num_triangles; i++){//SYSTEM->coordInfoVecs.nodeLocX.size(); i++) {
			if (SYSTEM->coordInfoVecs.triangles2Nodes_1[i] >= (INT_MAX-100) || 
			SYSTEM->coordInfoVecs.triangles2Nodes_2[i] >= (INT_MAX-100) ||
			SYSTEM->coordInfoVecs.triangles2Nodes_3[i] >= (INT_MAX-100) ||
			SYSTEM->coordInfoVecs.triangles2Nodes_1[i] < 0 ||
			SYSTEM->coordInfoVecs.triangles2Nodes_2[i] < 0 ||
			SYSTEM->coordInfoVecs.triangles2Nodes_3[i] < 0){continue;}
			 x = (SYSTEM->coordInfoVecs.nodeLocX[SYSTEM->coordInfoVecs.triangles2Nodes_1[i]] +
			 SYSTEM->coordInfoVecs.nodeLocX[SYSTEM->coordInfoVecs.triangles2Nodes_2[i]] +
			 SYSTEM->coordInfoVecs.nodeLocX[SYSTEM->coordInfoVecs.triangles2Nodes_3[i]])/3.0;
			 y = (SYSTEM->coordInfoVecs.nodeLocY[SYSTEM->coordInfoVecs.triangles2Nodes_1[i]] +
			 SYSTEM->coordInfoVecs.nodeLocY[SYSTEM->coordInfoVecs.triangles2Nodes_2[i]] +
			 SYSTEM->coordInfoVecs.nodeLocY[SYSTEM->coordInfoVecs.triangles2Nodes_3[i]])/3.0;
			 z = (SYSTEM->coordInfoVecs.nodeLocZ[SYSTEM->coordInfoVecs.triangles2Nodes_1[i]] +
			 SYSTEM->coordInfoVecs.nodeLocZ[SYSTEM->coordInfoVecs.triangles2Nodes_2[i]] +
			 SYSTEM->coordInfoVecs.nodeLocZ[SYSTEM->coordInfoVecs.triangles2Nodes_3[i]])/3.0;
			 chem = SYSTEM->coordInfoVecs.soln_per_triangle[i];
			 b = SYSTEM->coordInfoVecs.b_per_triangle[i];
			 c = SYSTEM->coordInfoVecs.u[i];
			ofs << std::setprecision(7) <<std::fixed<< x << " " << y << " " << z <<" "<< chem <<" "<< b <<" "<< c <<std::endl;
