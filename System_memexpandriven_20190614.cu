...
void System::solveSystem() {
	...
	std::vector<bool> used_triangles;
	used_triangles.resize(coordInfoVecs.triangles2Nodes_1.size(),false);
	...
	ljInfoVecs.Rmin_M = 1.5;
	ljInfoVecs.Rcutoff_M = 1.5;
	...
	ljInfoVecs.epsilon_LJ_rep1 = 15.0;//0.5;// 0.06;//7.5;
	ljInfoVecs.epsilon_LJ_rep2 = 0.9;//1.0;//1.0;//1.0;
	...
	int GROWTH_TIME = 1;
	int GROWTH_TIME_MEM = 100;
	int RECORD_TIME = 5;//round(Max_RunStep/2);
	...
	generalParams.eq_total_volume = generalParams.true_current_total_volume*1.25;//This is for setting different equilibrium volume to mimic growth or shirnkage.
 					...
 					while (current_time < Max_Runtime){
						...
						std::normal_distribution<double> distribution_noise(0.0, 0.01);
						 for (int i = 0; i < ljInfoVecs.LJ_PosX_all.size(); i++){
						   ...
						   if (i == (generalParams.maxNodeCountLJ-1)){
							double R = sqrt( 
...
if (edgeswap_iteration == 1 || edgeswap_iteration % GROWTH_TIME_MEM == 0){
...
for (int p = 0; p < VectorShuffleForGrowthLoop.size(); p++){
	//std::cout<<"p = "<<p<<std::endl;
		...
		if (generalParams.edges_in_upperhem[k] == -1){
			continue;
		}
		//int k = p;
		//k -= triggered_counter;
		...
		if (used_triangles[elem1] == false && used_triangles[elem2] == false){
			used_triangles[elem1] = true;
			used_triangles[elem2] = true;
		}
		else{
			continue;
		}
