...
void System::solveSystem() {
	...
	for (int i = 0; i < generalParams.maxNodeCount; i++){
		if (coordInfoVecs.nodeLocZ[i] > (generalParams.centerZ + 5.0)){
	...
	generalParams.volume_spring_constant = 10.0;//0.75;
	...
	double scale_linear = 75.0/15.0;
	double scale_bend = 75.0/7.5;//0.5*75.0;
	double scale_area = 75.0/5.0;//0.5*75.0;
...
for (int p = 0; p < VectorShuffleForGrowthLoop.size(); p++){
	//std::cout<<"p = "<<p<<std::endl;
		int k = VectorShuffleForGrowthLoop[p];//p;
		...
		/*if ((This_area_v/ areaTriangleInfoVecs.initial_area >= EXPAN_THRESHOLD) && (This_area_w/ areaTriangleInfoVecs.initial_area >= 0.5*EXPAN_THRESHOLD)){
            triggered = true;
			//triggered_counter += 1;
		}
		else if ((This_area_v/ areaTriangleInfoVecs.initial_area >= 0.5*EXPAN_THRESHOLD) && (This_area_w/ areaTriangleInfoVecs.initial_area >= EXPAN_THRESHOLD)){
            triggered = true;
			//triggered_counter += 1;
		}*/
